...
strinput=strinput.replace((/(\<\/span[^>]*\>)/gi),"");return strinput.replace((/(\<span[^>]*\>)/gi),"");},stripTags:function(strinput){if(!strinput){return""}
...
val=livespell.str.stripTags(val);return val;},setProxyHTML:function(id,val){livespell.insitu.proxyDOM(id).innerHTML=val;livespell.context.validate(id)},setProxyText:function(id,val){val=livespell.str.HTMLEnc(val);if(livespell.test.IE()){val="<p>"+val+"</p>"
val=val.replace(/<br\s*[\/]?>/gi,"</p><p>");val=val.replace(/<p>\s*<\/p>/gi,"<p>&#8203;</p>");}
this.setProxyHTML(id,val)},hasChanged:function(id){return E$(id).value.replace(/\t/gi," ")!=livespell.insitu.getProxyText(id).replace(/\t/gi," ");},safeUpdateProxy:function(id,providerId){if(!livespell.insitu.proxyDOM(id)||livespell.insitu.proxyDOM(id).hasFocus){return false}
...
this.setProxyText(id,value);},setProxyHTMLAndMaintainCaret:function(id,val){if(val===""||livespell.insitu.getProxyHTML(id)===val){return;}
var liveField=livespell.insitu.proxyDOM(id);if(livespell.test.IE()&&document.selection){var text=liveField.innerHTML;if(val.toLowerCase().indexOf("<p")==-1){val="<p>"+val+"</p>"}
...
clicky+=3;var scrolly=liveField.scrollTop;liveField.innerHTML=val;liveField.scrollTop=scrolly;try{cursorPos.moveToPoint(clickx,clicky)
...
text=tokens.join('');if(livespell.insitu.proxyDOM(id).hasFocus||livespell.context.isOpen()){this.setProxyHTMLAndMaintainCaret(id,text);}else{this.setProxyHTML(id,text);}}}},init:function(){if(livespell.insitu.initiated)return;livespell.insitu.initiated=true;livespell.context.renderShell()
livespell.events.add(window.document,"mousedown",livespell.context.hideIfNotinUse,false);livespell.events.add(window.document,"keydown",livespell.context.hide,false);livespell.userDict.load();livespell.context.hide();},allTextAreas:function(){var AF=[];oTAreas=document.getElementsByTagName("textarea")
